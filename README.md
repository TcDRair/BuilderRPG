...
시드 하나로 섬을 만들고, 그 위에 캐릭터를 세우고, 주변 사물과 상호작용하는 것까지가
현재 동작하는 범위입니다. 이 문서는 **무엇을 만들었고, 무엇이 아직 아닌지**를 한곳에 정리한 것입니다.
...
개인 프로젝트라 완성도가 영역마다 다릅니다. 그것을 감추면 읽는 쪽이 코드를 열어 보고서야 알게 되므로,
**구현 성숙도를 명시적으로 표기**했습니다.

| 등급 | 의미 |
|---|---|
| **동작** | 플레이 모드에서 검증됨. 테스트가 있는 경우 그것도 명시 |
...
**읽는 순서를 하나만 고르라면 [문서 01](docs/01-절차적-섬-생성.md)입니다.**
가장 완성도가 높고, 알고리즘·최적화·에디터 도구·테스트가 한 갈래에 다 들어 있습니다.
엔지니어링 쪽에 관심이 있다면 [문서 06](docs/06-엔지니어링.md)의 변이 테스트 절을 권합니다.
...
그래프 구축 · 고도 전파 · 강 · 습도 · 바이옴 분류의 **알고리즘 골격은 선행 작업의 것**입니다.

제가 한 것은 "50×50 고정 크기의 2D 데모"를 "가변 크기의 3D 플레이 가능 지형을 뽑는 도구"로
바꾸는 작업입니다 — 파이프라인 코루틴화, 목표 육지 비율 자동 수렴, 채널 패킹, Terrain 변환기,
에디터 워크플로 등입니다. 문서 01에서 ★ 표시로 구분해 두었습니다.

**그 외 영역은 전부 직접 설계·구현했습니다.**

---

## 3. 시스템 구성과 결합 관계

각 시스템이 어떻게 맞물리는지가 개별 문서에는 나오지 않으므로 여기에 정리합니다.
...
실선은 실제로 연결되어 동작하는 경로이고, **점선은 아직 이어지지 않은 곳**입니다.
셋 다 이유가 다릅니다.

| 끊긴 지점 | 사정 |
|---|---|
| 지형 프롭 → 상호작용 | `Rair.Field.Maps.Prop`(배치 정보 struct)과 `Rair.Field.Interact.Prop`(상호작용 MonoBehaviour)은 **이름만 같고 무관한 타입**입니다. 지형이 심은 나무는 아직 클릭할 수 없습니다 |
| 점유 격자 → 건축 | 소비처인 건축 시스템이 유실 상태입니다. 그래서 의미만이라도 테스트로 못박아 두었습니다 |
| 아이템 → 상호작용 | 아이템 실물이 하나도 정의되어 있지 않습니다 |

첫 번째는 이름 충돌이 혼란을 부르는 사례이기도 합니다.
지형 쪽을 `PropPlacement` 정도로 개명하는 것이 맞습니다.
...
기술적으로 볼 만한 것은 세 가지입니다.

- **목표 육지 비율 자동 수렴** — 이진 탐색으로 노이즈 임계값을 조정합니다.
  그런데 수렴하지 않고 있었고, **테스트가 그것을 찾아냈습니다** (문서 01의 4.3절)
- **채널 패킹** — 고도·습도·바이옴·육지여부를 RGBA 4채널에 인코딩해 텍스처 하나로 넘깁니다
- **멈추지 않는 실행** — 전 과정이 코루틴이고 `ProgressTimer` 추상화가 `[2/4]` 형태로 전체 진행률을 표시합니다

→ [문서 01](docs/01-절차적-섬-생성.md)

### 4.2 스탯 · 버프 · 어빌리티 — **동작**

"이동속도"라는 값 하나에 붙는 요구사항 — 상하한, 가산·승산 수정자, 정확한 원복,
순서 무관성, 다른 스탯과의 연동, UI에 계산 과정 노출 — 을 `float` 필드로는 감당할 수 없어
**값의 종류를 타입 계층으로 나눈** 결과입니다.

핵심은 `*`를 곱셈이 아니라 `Multiplier += (f - 1)`로 정의한 것입니다.
가산기와 승산기가 독립 누산기에 쌓이므로 **교환법칙이 성립하고 역연산이 정확히 상쇄됩니다.**
이 두 성질은 테스트로 고정되어 있습니다.

→ [문서 02](docs/02-스탯-버프-시스템.md)

### 4.3 상호작용 · 작업 큐 — **부분 동작**

오브젝트를 클릭하면 가능한 상호작용이 뜨고, 고르면 캐릭터가 스스로 다가가 수행합니다.

설계의 축은 **작업 큐를 유닛이 아니라 대상이 소유한다**는 것입니다.
...
방향은 흔하지만 **URP에서는 빌트인 시절의 방법이 하나도 통하지 않아** 제약 셋을 차례로 우회해야 했습니다 —
...
세 갈래가 각각 다른 이유로 멈춰 있습니다.
아이템은 물성 모델(질량·부피·온도·열용량)과 병합 엔진이 있으나 **아이템 실물이 없고**,
NPC는 키워드 지식 표시기가 동작하나 **그것을 쓸 NPC가 비어 있으며**,
건축은 코드가 커밋 `0d8ef958`에 남아 있으나 **의존 프리팹의 메시가 유실**되었습니다.

공통점이 있습니다 — **모델을 먼저 만들고 사용처를 나중에 두었습니다.**
반대 순서였던 건축만 한때 실제로 동작했습니다.
...
게임 콘텐츠가 아니라 프로젝트를 굴러가게 만든 쪽의 작업입니다.
...
**변이 테스트로 테스트를 검증했습니다.** 사후에 쓴 테스트가 통과하는 것만으로는
아무것도 증명되지 않으므로, 고쳤던 결함을 일부러 다시 넣어 실제로 잡히는지 확인했습니다.
잡지 못한 변이 2건도 그대로 기록하고, 그것이 테스트의 구멍이 아니라
**변이가 동작을 바꾸지 않은 경우**임을 분석해 두었습니다.

**에디터 어셈블리에 씬 컴포넌트를 두면 안 된다는 것을 실측으로 확인했습니다.**
플레이 모드를 오갈 때 직렬화 값이 전부 기본값으로 초기화되는데,
...
복구는 git 이력에서 GUID를 역추적하는 방식으로 진행했고,
**되살릴 수 없는 것과 되살린 것을 구분해 기록**했습니다.
이 문서에서 건축 시스템이 "유실"로 표기되는 이유가 여기에 있습니다.
...
다음 작업의 우선순위와 착수 조건은 [남은 작업](docs/internal/남은-작업.md)에 정리되어 있습니다.
가장 값이 큰 것은 **건축 시스템 복원**입니다 — 그것 하나가 미결 항목 넷을 물고 있습니다.
