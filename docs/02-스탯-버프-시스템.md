...
전 스탯을 대조한 결과 실제로 걸리는 것은 **하한**이었습니다.
...
조합으로 넘는 것은 의도가 아니므로, **클램프가 고치는 결함**으로 판정했습니다.

> 2.2절의 합연산 스태킹은 순서 무관성을 얻는 대신
> **배율 합이 음수가 될 수 있다**는 성질을 함께 가집니다.
...
중첩 기본 규칙은 **갱신**이고, `Duration`만 대조해 긴 쪽을 남깁니다
...
아무것도 증명되지 않으므로 **변이 테스트**로 유효성을 확인했습니다.
