...
>
> 구현 성숙도 — 동작. 테스트 27케이스로 고정되어 있습니다.
...
즉 상한이 `최대HP − 제한량`이라는 다른 두 값의 함수입니다.

접근. min/max를 상수가 아니라 참조 + 변환 함수로 받습니다.
...
세 스탯의 의존 관계가 생성자 세 줄에 선언적으로 표현됩니다.
...
값을 세 구조체로 나눴습니다. 나누는 기준은 누가 쓰느냐입니다.
...
`UnitInfo`의 계수들은 단위와 의미를 주석이 아니라 생성자 인자로 못박았습니다.
...
앞의 세 절이 "값을 어떻게 표현할 것인가"였다면, 이 절은 그 표현이 실제로 무엇을 감당해야 했는가입니다.
...
그러면 "플레이어가 원하는 것"과 "지금 가능한 것"의 교집합 중 가장 빠른 것이
곧 현재 상태가 됩니다.
...
SP가 바닥날 때 뛰기/걷기가 매 프레임 번갈아 바뀌는 채터링을 막아야 합니다.
임계값을 하나만 두면 반드시 발생합니다.

진입과 이탈에 서로 다른 임계값을 씁니다.
...
SP가 분 단위의 "숨"이라면, 피로는 시간 단위의 누적 소모입니다.
...
「산들바람」은 하중 구간 판정을 4단계에서 2단계로 재정의합니다.
...
최대 하중은 크게 깎이지만(`-80~90%`), 그 안에서는 항상 경량 취급을 받습니다.
"가볍게 다니는 대신 짐을 못 든다"는 컨셉이 수치 보정이 아니라
규칙 교체로 표현됩니다. 수치만 만지는 버프로는 나올 수 없는 종류의 효과입니다.
...
해제 시점에 소모한 HP를 되돌리는 별도 이펙트를 부착합니다.
...
쌓기는 초당 1, 잃기는 초당 최대치의 20%로 비대칭입니다.
...
세 번째 줄이 요점입니다. `Light(u)` / `Over(u)`가 현재 하중 상태를 보고
...
UI 쪽은 효과 목록과 UI 목록을 양방향으로 diff해 동기화합니다.
...
하지만 `stat.RunSpeed *= 1.3f`는 기획 문서의 표기와 그대로 일치합니다.
...
어빌리티가 아니라 유닛 클래스를 상속해야 바꿀 수 있습니다.
...
| 당시 상태 | 현재 |
|---|---|
| `RAFloat`이 최종 클램프를 하지 않음 | 적용기 체인을 모두 거친 뒤 클램프 |
| `Ability` 인스턴스가 가변 상태 보유 | `CreateEffect(unit)` 팩터리로 유닛마다 생성 |
| 효과 중첩·갱신 미구현 | 갱신을 기본 규칙으로 구현 |
| `Stack` 초기값 `-1`이 계산식에 유입 | `Begin()`에서 스택 사용 효과만 0으로 초기화 |
| 적용기 순서에 우선순위 없음 | 순서 의존성을 실행 시점에 검출 |
| `UnitStat`/`UnitInfo` 가변 struct | 보류 — 근거는 아래 |
| 테스트 없음 | `RValue` 12건 + 효과 검증 도입 |

### 클램프가 실제로 잡은 것

`RAFloat.Value`가 체인을 모두 거친 뒤 선언 범위로 클램프합니다.

```csharp
public override float Value => Nullify ? 0 : Mathf.Clamp(RVValue.Value, Min, Max);
```

`RVValue`는 클램프 이전 원본을 유지합니다 — 체인 중간값은 잘리면 안 되고,
범위를 얼마나 넘어섰는지 알아야 하는 곳이 있을 수 있기 때문입니다.

이 문서가 예로 들었던 `RunSpeed`는 사실 문제가 아니었습니다.
기본 5에 「산들바람」 2레벨의 ×1.2로 6이 되어 상한 20에 닿지 않습니다.
전 스탯을 대조한 결과 실제로 걸리는 것은 하한이었습니다.

| 단계 | 결과 |
|---|---|
| `RunSPCost = new(6, 0)` | 기본 6, 하한 0 |
| 「산들바람」 — `*= 0.5` | 배율 0.5 |
| 「러너스 하이」 — `*= SP` | `SP = 1 - 0.02 × Stack`, 2레벨 최대 스택 50 → `SP = 0` |
| 배율 합 | `0.5 + (0 - 1) = -0.5` |
| 값 | `6 × (-0.5) = -3` |

`Tick()`이 `stat.SP += (SPRegen - SPCost) * dt`이므로
소모값이 음수면 달릴수록 SP가 찹니다.
두 어빌리티를 동시에 켜고 50초 연속 질주해야 도달하는 조합이라 드러나지 않았습니다.

단일 효과만으로는 범위를 넘지 않도록 설계되어 있었고
조합으로 넘는 것은 의도가 아니므로, 클램프가 고치는 결함으로 판정했습니다.

> 2.2절의 합연산 스태킹은 순서 무관성을 얻는 대신
> 배율 합이 음수가 될 수 있다는 성질을 함께 가집니다.
> 그 성질을 막는 것이 선언된 범위인데, 정작 그 범위가 적용되지 않고 있었습니다.

### 상태의 소유자

`Ability.Effect`(단일 공유 인스턴스)를 `Ability.CreateEffect(unit)` 팩터리로 바꿔
효과를 유닛마다 만듭니다. 진행 상태는 `UnitEffect`를 상속한 `State`가 가집니다.

| 어빌리티 | 옮긴 상태 |
|---|---|
| 「러너스 하이」 | `trigger` → `State.running`, `elapsed` → `State.elapsed` |
| 「무쇠걸음」 | `rcr` → `State.restricted`, `hp_trigger` → `State.consuming` |
| 「산들바람」 | 없음 (`UnitEffect` 그대로) |

`FieldUnit.effects`가 이미 유닛별이므로 이것만으로 공유 문제가 사라집니다.
중첩 기본 규칙은 갱신이고, `Duration`만 대조해 긴 쪽을 남깁니다
(`-1`은 무기한이라 어느 쪽이든 `-1`이면 무기한). 효과별로 재정의할 수 있습니다.

### 순서 의존성 — 차단 대신 검출

구조적 차단은 포기했습니다.
`Default_RunSpeed`와 `Default_WalkSpeed`가 승가산 외에 `Nullify`를 제3의 채널로 쓰고,
그중 하나는 진입 시 `Nullify = false`로 지우기까지 합니다.
교환법칙이 성립하는 형태로 만들려면 `Nullify`의 의미부터 재정의해야 합니다.

대신 적용기 체인을 역순으로도 접어 보고 결과가 다르면 오류를 냅니다.
적용기가 2개 이상일 때만, 에디터·개발 빌드에서만 동작합니다.
컴파일러가 막을 수 없는 계약이라 실행으로 검사하는 방식입니다.

> 사실 확인 — 적용기가 둘 이상 붙은 `RAFloat`는 현재 하나도 없습니다.
> 등록 지점 7곳이 전부 서로 다른 스탯이라 순서 문제는 아직 발생한 적이 없습니다.

### 가변 struct는 보류

전환하지 않기로 했습니다. 근거는 세 가지입니다.

1. 실제 결함이 없습니다. 복사 지점이 `FieldUI` 두 줄뿐이고 전부 읽기 전용입니다.
2. 클래스 전환이 새 위험을 만듭니다. `UnitStatus`는 어디서도 대입되지 않고
   struct의 0 초기화에 의존합니다. 클래스로 바꾸면 무해한 기본값이 `null` 참조 오류가 됩니다.
3. 얻는 것이 가상의 미래 버그 예방인데, 더 싼 수단으로 대부분 막을 수 있습니다.

대신 `FieldUI`의 복사를 `ref var s = ref P.stat;`으로 바꿔
올바른 접근 패턴을 코드에 남겼고, 보류 사유를 주석으로 명시했습니다.

### 테스트가 증명한 것

`RValue` 12개 케이스를 작성하면서, 사후 테스트가 통과하는 것만으로는
아무것도 증명되지 않으므로 변이 테스트로 유효성을 확인했습니다.

| 의도적으로 넣은 결함 | 잡아낸 테스트 |
|---|---|
| `RVFloat.Value`에 최종 클램프 추가 | 특성화 2건 |
| `operator +`를 `Adder += f * Multiplier`로 변경 | 순서 무관 2건 |

2.2절이 주장한 두 성질(정확한 역연산·순서 무관)이
실제로 회귀를 잡는다는 것이 이걸로 확인됩니다.
「무쇠걸음」 3레벨의 `/= 0` 복원 경로도 테스트에 포함했습니다 —
진짜 곱셈이었다면 여기서 정보가 소실됩니다.
...
- `Ability.ID`가 직렬화에 부적합합니다.
  ...
  세이브 포맷이 정해지지 않아 명시적 ID 체계를 두지 않은 상태입니다.
- 데이터가 전부 코드에 하드코딩되어 있습니다.
  `Abilities.Initialize()`가 비어 있고 어빌리티 3종이 C# 클래스로 작성돼 있습니다.
  발동도 디버그 키(`F5`~`F10`)로만 가능합니다.
  데이터 스키마는 위 항목의 안정적 ID에 종속됩니다.
- `Nullify`가 제3의 채널로 쓰이고 있습니다.
  승가산과 별개로 값을 0으로 만드는 경로인데, 적용기가 이를 읽고 쓰기까지 해서
  순서 의존성의 구조적 차단을 막는 원인이 되고 있습니다.
