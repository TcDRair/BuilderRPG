...
보정은 최대 9회 일어나고 그동안 탐색 폭이 0.25에서 약 0.001까지 좁혀지므로,
실제로는 오차 1% 조건에 훨씬 먼저 걸려 빠져나옵니다.
...
포트폴리오라고 좋은 것만 적을 이유는 없으니, 현재 상태를 그대로 적습니다.

- **에디터 전용.** `RandomTextureGenerator`와 `TerrainGenerator`가
  `#if UNITY_EDITOR`로 묶여 있어 런타임 생성이 불가능합니다.
  `EditorCoroutine` 의존을 걷어내면 되지만 아직 하지 않았습니다.
- **블러가 in-place.** 하이트맵 블러는 이웃 평균을 같은 배열에 바로 씁니다.
  스플랫맵 블러는 별도 버퍼를 할당해 두고도 매 반복 끝에 한 방향으로만 대입해서,
  반복 2회차부터는 역시 in-place로 동작합니다. 둘 다 주사 방향으로 미세한 편향이 생깁니다.
  버퍼를 교대로 맞바꾸면 해결되지만, 생성 결과가 달라지므로 파라미터 재조정과 함께 다뤄야 합니다.
- **채널 주석 불일치.** `MapTexture`의 채널 설명 주석이 실제 인코딩과 어긋나 있습니다
  (주석은 `B:occupy, A:biome`, 실제는 `B:biome, A:land`).
  소비하는 쪽은 실제 인코딩을 따르므로 동작에는 문제가 없습니다.
  해당 파일이 포크한 별도 저장소에 있어 아직 고치지 않았습니다.
- **점유 그리드의 의미가 미정.** 축소 변환은 합집합으로 정리했지만,
  호출부가 육지를 `FULL`(완전 점유)로 매핑하고 있어
  "육지 = 건축 가능"인지 "육지 = 점유됨"인지가 아직 정해지지 않았습니다.
  이 값을 소비하는 코드가 없어 드러나지 않은 상태입니다.
- **테스트 없음.** 생성 결과의 성질(육지 비율 수렴, 강이 반드시 해안에 도달, 고도 범위)은
  모두 속성 기반 테스트로 검증하기 좋은 대상인데 작성하지 않았습니다.
