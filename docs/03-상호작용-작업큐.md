...
세 번째가 특히 성가신 부분입니다. "다가가서 캔다"는 한 문장이지만,
그 사이에 플레이어가 다른 곳을 클릭하거나 대상이 사라지는 경우를 전부 처리해야 합니다.
...
**유닛이 아니라 대상에 둔 이유**는 자원 경합이 대상 단위로 일어나기 때문입니다.
...
갱신 전략은 **매 프레임 전체 재계산 후 차이만 반영**입니다.
...
**이 방식을 택한 이유**는 상호작용 목록이 런타임에 바뀔 수 있기 때문입니다.
...
슬롯이 한 자릿수인 지금은 문제가 되지 않지만, 갱신 트리거를 두는 편이 옳습니다. 7절에 남겼습니다.
...
코루틴을 택한 것은 **취소가 한 줄로 끝나기 때문**입니다 — `StopCoroutine` 하나면
...
**프롭을 껐다 켜면 파괴·수리 슬롯이 두 벌이 됩니다.** 지금 씬에서 프롭을 토글하는 경로가 없어
드러나지 않았을 뿐입니다. `slots.Clear()`를 앞에 두거나 `Awake`로 옮겨야 합니다.
...
게다가 같은 F8이 `Player.Update`에서 어빌리티 해제에도 쓰이고 있어 키가 겹칩니다.
취소는 UI 버튼(`InteractTaskUI.Cancel`)이 이미 제공하므로, 이 임시 단축키는 걷어내는 것이 맞습니다.
...
- 그 필드들을 복원하거나, 현재의 콜백 방식(`IAProgress`의 `Act` 델리게이트)에 맞게 다시 설계
- 파일 위치 이동 — `Prop` 본체는 런타임 어셈블리(`Rair.Runtime`)에 있어 `UnityEditor`를 참조할 수 없습니다.
  에디터 코드는 `Rair.Editor`로 가야 합니다

**지금은 상호작용을 코드로만 정의할 수 있습니다.** 데이터 주도가 아니라는 뜻입니다.
...
| `IAProgress.Type.Special` | 전투·건설용. **건축 시스템이 현재 부재**라 소비처가 없습니다 |
...
- `Interaction.DestroySprite` 등이 `Resources.Load`를 **프로퍼티 접근마다** 호출합니다. 캐시해야 합니다
...
셋 다 현재 규모에서는 측정되는 비용이 아닙니다. 프롭 수가 늘면 순서대로 문제가 됩니다.
