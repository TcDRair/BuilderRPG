...
>
> 게임 콘텐츠가 아니라 프로젝트 기반을 정비한 작업의 기록입니다.
> 이 단계의 작업과 문서화는 AI 도구를 사용해 진행했습니다.
> 게임 시스템 구현(문서 01~05)은 직접 작성한 코드입니다.

## 1. 개요

빌드 실패, 회귀 검출 수단 부재, 에셋 참조 손상 — 세 방향으로 대응했습니다.
...
| `TerrainGenerator.cs` | `using UnityEditor;`가 `#if UNITY_EDITOR` 바깥에 위치 |
...
컴파일러가 잡아야 할 문제를 검색으로 잡으려 한 것이 원래의 문제였습니다.
...
가드가 표현하던 사실이 어셈블리 경계로 표현되므로, 위반하면 컴파일이 실패합니다.
...
부수 효과로 인게임 맵 생성 경로가 열렸습니다.
생성 로직이 런타임 어셈블리에 있으므로 에디터 없이도 호출할 수 있고,
현재 남은 에디터 의존은 구동부의 `EditorCoroutine`뿐입니다.
...
그래서 특정 값이 아니라 어떤 시드에서도 성립해야 하는 불변 조건만 검사합니다.
...
지금은 `Biome` 열거형을 직접 순회하므로 바이옴을 추가하고 가중치 행을 빠뜨리면
...
이 중첩을 펼치는 것은 Unity 런타임 스케줄러의 일이라 EditMode에는 없습니다.
...
이것은 테스트의 구멍이 아니라 변이가 동작을 바꾸지 않았다는 뜻입니다.
