...
동작하지 않는 세 갈래를 한곳에 모았습니다.
각 절은 무엇을 하려 했는가, 어디까지 있는가, 왜 멈췄는가를 다룹니다.
...
아이템을 "이름 붙은 데이터 덩어리"가 아니라 물리적 성질의 집합으로 두려 했습니다.
그러면 제작 결과를 레시피마다 손으로 적지 않고 규칙에서 유도할 수 있습니다.
...
각 속성이 자기 병합 방식을 알고 있다는 것이 설계의 핵심입니다.
...
속성을 자유롭게 조합하게 두면 모순된 아이템이 만들어집니다 —
...
`Activity`의 주석이 정확한 진단입니다. 욕구 수치로 행동을 고르는 것까지는 설계가 되는데,
고른 행동을 실행하는 것은 상호작용 시스템과 맞물려야 합니다.
...
건축 시스템이 소비처였던 항목 넷이 미결로 남아 있습니다.

| 항목 | 미결 사유 |
|---|---|
| `Occupy` ↔ `Buildable` 대응 | 플래그 구성이 거의 일대일이지만, 소비처가 없어 확정하지 못했습니다 |
| `MainCamera.blockingLayers` 선정 | 건축물이 원래의 가림 대상이었습니다 ([문서 04](04-URP-가림-반투명화.md)) |
| 반투명화 실제 검증 | 원래 대상 대신 임시 Lit 큐브로 확인했습니다 |
| `IAProgress.Type.Special` | 전투·건설용인데 건설이 없어 소비처 0건입니다 ([문서 03](03-상호작용-작업큐.md)) |
...
`EventProperty`의 `MergeType` 7종 중 실제로 실행된 것이 몇 개인지 확인할 수단이 없습니다.
