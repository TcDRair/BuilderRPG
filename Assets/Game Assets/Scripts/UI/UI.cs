...
namespace Rair.Field
{
	public class UI : MonoBehaviour
	{
		...
		[SerializeField] protected CanvasGroup buildingModeMenu, buildingSelectionUI, nameTagUI, interactionSelectUI;
		...
		void ShowInteractableNameTags()
		{
			...
			{
				var interactable = hit.transform.GetComponent<Prop>();
		...
		RectTransform CreateNameTag(Prop interactable, Vector3 pos)
		{
		...
		public void ShowInteractions(Prop interactable)
		{
			...
			if (interactable.Interactable) for (int i=0; i<interactSlots.Length; i++) {
				...
				//* 슬롯 필수 구성요소 표시
				var slot = interactable.Slots[i];
				InteractSlotUI slotUI;
				if (slot.type == Interaction.Type.Small) {
