...
namespace Unity.EditorCoroutines.Editor
{
  ...
  public class EditorCoroutine
  {
    ...
    {
      if ((m_hasOwner && !m_Owner.IsAlive) || AllStop) { EditorApplication.update -= MoveNext; return; }

      bool done = ProcessIEnumeratorRecursive(m_Routine);
      m_IsDone = !done;

      if (m_IsDone) EditorApplication.update -= MoveNext;
    }

    static readonly Stack<IEnumerator> kIEnumeratorProcessingStack = new(32);
